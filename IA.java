...
public class IA {
    ...
    public static char bestMove(char couleurPlayer){
        ...
        for (int i=0;i < Main.panneau.len;i++){
             for (int j=0; j< Main.panneau.len; j++){
                 
                 for(int i1=-1; i1<2; i1++){
                     for(int j1=-1; j1<2; j1++){
                         try{
                             if(Main.panneau.grille[i+i1][j+j1] == couleurPlayer && i != i1 && j != j1){
                                 switch(Main.panneau.grille[i][j]){
                                     case 'r':
                                         couleursAutour[0] ++;
                                         break;
                                     case 'v':
    ...
    public static char maximum(int[] tableau){
        ...
        for (int m=0; m<6; m++){
            if (tableau[m] > max && Main.Couleur(couleurs[m], Main.courantPlayer)){
                max = tableau[m];
                indice_max = m;
            }
        }
